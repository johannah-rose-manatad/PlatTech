...
const supabaseUrl = 'https://ssakzlvbipegkdvqwiph.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNzYWt6bHZiaXBlZ2tkdnF3aXBoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzI1MTM3NjIsImV4cCI6MjA0ODA4OTc2Mn0.OZOpD_KiHuxztDXILXEG80kGLl6J0nknV-4b7zTPldk';
...
function goToMonthYear() {
    ...
    if (monthInput && yearInput) {
        const monthIndex = parseInt(monthInput) - 1; // Months are 0-indexed
...
async function saveTask(button) {
    const taskContainer = button.parentNode; // Entire task input container
    ...
    if (error) {
        ...
        // Remove the input box and save button container entirely
...
async function buttonPressSignup() {
    ...
    let { data, error } = await connection
        .from('user_info')
        .insert([ { username: name, password: passWord, email: email } ]);
